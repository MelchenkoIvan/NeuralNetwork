...
public static class Executor
{
    ...
    public static double Predict(List<double[,]> inputs)
    {
        var tacts = new List<Tact>();
        for (var i = 0; i < inputs.Count; i++)
        {
            var inputSignals = inputs[i];
            tacts.Add(new Tact(i, DataSetHelper.Scalling(inputSignals).ToListArrays().Single()));
        }

        return LearnHeartSickModel().RecurrentPredict(tacts).Output;
    }
